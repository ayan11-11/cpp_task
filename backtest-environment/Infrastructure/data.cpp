...
void HistoricalCSVDataHandler::format_csv_data() {
    ...
    for(int i=0;i<symbol_list->size();i++) {
        ...
        cout << string("symbol_list :") << (*symbol_list)[i];
        ...
        MarketDataFrame moves = YahooFinanceCSVReader((char*)symbol.c_str(),
                                                      (char*)(string("./DataHandling/CSV directory/") + string("XBTUSD-1d-data-latest") + string(".csv")).c_str()).marketmovements;
        ...
        MarketDataFrame movesbuffer = YahooFinanceCSVReader((char*)symbol.c_str(),
                                                            (char*)(string("./DataHandling/CSV directory/") + string("XBTUSD-1d-data-old") + string(".csv")).c_str()).marketmovements;
...
// tuple<string, long, double, double, double, double, double, double, double, double, double, double, double> HistoricalCSVDataHandler::get_new_bar(string symbol) {
    ...
    // tuple<string, long, double, double, double, double, double, double, double, double, double, double, double>lastbar;
    ...
    if (symbol_data[symbol]["open"][date] != 0) {
        lastbar = make_tuple(symbol, date, symbol_data[symbol]["open"][date],symbol_data[symbol]["low"][date],symbol_data[symbol]["high"][date],
                             symbol_data[symbol]["close"][date],symbol_data[symbol]["vwap"][date],symbol_data[symbol]["volume"][date]);
        // lastbar = make_tuple(symbol, date, symbol_data[symbol]["open"][date], symbol_data[symbol]["high"][date], symbol_data[symbol]["low"][date], symbol_data[symbol]["close"][date],
        //                                    symbol_data[symbol]["trades"][date], symbol_data[symbol]["volume"][date], symbol_data[symbol]["vwap"][date], symbol_data[symbol]["lastSize"][date], 
        //                                    symbol_data[symbol]["turnover"][date], symbol_data[symbol]["homeNotional"][date], symbol_data[symbol]["foreignNotional"][date]);
...
map<string, map<long, double>> HistoricalCSVDataHandler::get_latest_bars(string symbol, int N=1) {
    ...
    for (int i = N; i >= 0; i--) {
        ...
        bars_list["vwap"][date] = latest_data[symbol]["vwap"][date];
        bars_list["volume"][date] = latest_data[symbol]["volume"][date];
        // bars_list["open"][date] = latest_data[symbol]["open"][date];
        // bars_list["high"][date] = latest_data[symbol]["high"][date];
        // bars_list["low"][date] = latest_data[symbol]["low"][date];
        // bars_list["close"][date] = latest_data[symbol]["close"][date];
        // bars_list["trades"][date] = latest_data[symbol]["trades"][date];
        // bars_list["volume"][date] = latest_data[symbol]["volume"][date];
        // bars_list["vwap"][date] = latest_data[symbol]["vwap"][date];
        // bars_list["lastSize"][date] = latest_data[symbol]["lastSize"][date];
        // bars_list["turnover"][date] = latest_data[symbol]["turnover"][date];
        // bars_list["homeNotional"][date] = latest_data[symbol]["homeNotional"][date];
        // bars_list["foreignNotional"][date] = latest_data[symbol]["foreignNotional"][date];
    }
...
void HistoricalCSVDataHandler::update_bars() {
    ...
    for (int i=0; i < symbol_list->size(); i++) {
        ...
        // tuple<string, long, double, double, double, double, double, double, double, double, double, double, double> updateData = get_new_bar(symbol);
        ...
        latest_data[get<0>(updateData)]["vwap"][get<1>(updateData)] = get<6>(updateData);
        latest_data[get<0>(updateData)]["volume"][get<1>(updateData)] = get<7>(updateData);

        // latest_data[get<0>(updateData)]["open"][get<1>(updateData)] = get<2>(updateData);
        // latest_data[get<0>(updateData)]["high"][get<1>(updateData)] = get<3>(updateData);
        // latest_data[get<0>(updateData)]["low"][get<1>(updateData)] = get<4>(updateData);
        // latest_data[get<0>(updateData)]["close"][get<1>(updateData)] = get<5>(updateData);
        // latest_data[get<0>(updateData)]["trades"][get<1>(updateData)] = get<6>(updateData);
        // latest_data[get<0>(updateData)]["volume"][get<1>(updateData)] = get<7>(updateData);
        // latest_data[get<0>(updateData)]["vwap"][get<1>(updateData)] = get<8>(updateData);
        // latest_data[get<0>(updateData)]["lastSize"][get<1>(updateData)] = get<9>(updateData);
        // latest_data[get<0>(updateData)]["turnover"][get<1>(updateData)] = get<10>(updateData);
        // latest_data[get<0>(updateData)]["homeNotional"][get<1>(updateData)] = get<11>(updateData);
        // latest_data[get<0>(updateData)]["foreignNotional"][get<1>(updateData)] = get<12>(updateData);
